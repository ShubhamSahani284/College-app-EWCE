...
// Function to retrieve the login USN from SharedPreferences
Future<String> _getLoginID() async {
  final prefs = await SharedPreferences.getInstance();
  return prefs.getString('loginID') ?? ''; // Replace 'loginID' with the actual key
}
class MyHomePage extends StatelessWidget {
  Future<Map<String, dynamic>> _getUserData() async {
    final loginID = await _getLoginID();
    final userDoc = await FirebaseFirestore.instance.collection('Details').doc(loginID).get();
  ...
  Widget build(BuildContext context) {
      ...
      appBar: AppBar(
        ...
        title: FutureBuilder<String>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasData) {
              return Text("USN : ${snapshot.data}");
            } else if (snapshot.hasError) {
              return Text("Error: ${snapshot.error}");
            } else {
              return Text("Loading..."); // Show a loading message while fetching data
            }
          },
        ),
    ...
    Text(
    'Dept: ${userData['dept']}     Sem: ${userData['sem']}',
    style: TextStyle(fontSize: 16.0),
    ),
    Text(
    ...
    return Center(child: Text("Loading...")); // Show a loading message while fetching data
    }
    },
    ),
      Container(
